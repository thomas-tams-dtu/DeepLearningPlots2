...
val_p_d <-dense_data |>
  ...
  labs(title = 'Standalone DNN parameter search',
...
val_p_VAE100 <- VAE_data_100 |>
  ...
  theme_minimal() +
  labs(title = 'VAE parameter search',
       subtitle = 'Trained on archs4 without early stopping\nwith 100 epochs',
       fill = 'Average of\nlast 10\nvalidation loss',
       x = 'Latent Features',
       y = 'Beta')
...
lowest_val_PCAD
breaks_vector <- c(lowest_val_PCAD, 0.38, 0.45, 0.55, 0.61, 0.67)
limits_vector <- c(lowest_val_PCAD,0.7)

PCAD_tester <- PCAD_data_avg_test |>
  ggplot(aes(x = latent_features, y = weight_decay, fill = val_min)) +
  geom_tile() +
  scale_fill_gradientn(colours=color_vector,
                       na.value = "grey",
                       breaks=breaks_vector,
                       limits=limits_vector,
                       labels = scales::label_number(accuracy = 0.01)) +
  facet_wrap(~network_size) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    labs(title = 'PCA-Dense network',
       fill = validation_fill_text,
       x = 'PCA size',
       y = 'Weight decay L2') +
  guides(fill = "none")

encD_tester <- EncD_data_avg_test |>
  ggplot(aes(x = latent_features, y = weight_decay, fill = val_min)) +
  geom_tile() +
  scale_fill_gradientn(colours=color_vector,
                       na.value = "grey",
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title = 'Encoder-Dense network',
       subtitle = ,
       fill = validation_fill_text,
       x = 'Encoder latent features') +
    theme(axis.title.y = element_blank())

combined_test <- PCAD_tester + encD_tester
combined_test <- combined_test +
  plot_annotation(title = 'Network parameter search',
                  subtitle = 'VAE trained on archs4 & Dense network trained on gtex')

combined_test

ggsave('combined_test.png',
       combined_test,
       width=8,
       heigh=4)

#combined_all <- (val_p_VAE100 + val_p_d) / combined_test

#combined_all

#ggsave('combined_all.png',
#       combined_all,
#       width=8,
#       heigh=8)
...
plot1 <- ggplot() + geom_line(aes(x, y1), color = 'blue') + ggtitle('sin(x)')
plot2 <- ggplot() + geom_line(aes(x, y2), color = 'red') + ggtitle('cos(x)')

# Combine plots using patchwork
combined_plot <- plot1 + plot2 + plot_layout(ncol = 1, heights = c(1, 1)) +
...
VAE_data <- VAE_all |>
  filter(
    str_detect(network_name, e15) & str_detect(network_name, lr) &  str_detect(network_name, lf2) |
    str_detect(network_name, e30) & str_detect(network_name, lr) & !str_detect(network_name, lf2)
    ) |>
  filter(latent_features == 2)
